...
def get_dist_logger(name: str = "my_logger") -> DistributedLogger:
    ...
    Returns:
        :class:`DistributedLogger`: A distributed logger singleton instance.
    """
    return DistributedLogger.get_instance(name=name)


def disable_existing_loggers(include: Optional[List[str]] = None, exclude: List[str] = ["my_logger"]) -> None:
    """Set the level of existing loggers to `WARNING`. By default, it will "disable" all existing loggers except the logger named "my_logger".
    ...
    Args:
        ...
        exclude (List[str], optional): Loggers whose name not in this list will be disabled.
            This argument will be used only when `include` is None. Defaults to ['my_logger'].
    """
    if include is None:
