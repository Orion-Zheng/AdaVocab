# source: https://github.com/catid/dora/blob/main/dora.py
...
class DoRALayer(nn.Module):
    def __init__(self, d_in, d_out, rank=4, weight=None, bias=None):
        ...
        # m = Magnitude column-wise across output dimension (1, d_in)
    ...
    def forward(self, x):
        # lora_a.shape: (d_out, rank), lora_b.shape: (rank, d_in)
        # lora.shape: (d_out, d_in), self.weight.shape: (d_out, d_in)   
        lora = torch.matmul(self.lora_A, self.lora_B)  
        adapted = self.weight + lora
        column_norm = adapted.norm(p=2, dim=0, keepdim=True)  # Column-wise norm, column.shape: (1, d_in)
        norm_adapted = adapted / column_norm
        calc_weights = self.m * norm_adapted
        # y = xA^T + b: x.shape: (..., d_in), A.shape: (d_out, d_in) --> y.shape: (..., d_out)
